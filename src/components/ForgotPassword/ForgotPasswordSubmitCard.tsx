...
function ForgotPasswordSubmit({
...
}: ForgotPasswordSubmitProps): ReactElement {
	...
	const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		if (password !== confirmPassword) {
			setAlert('The password and confirmation password do not match. ');
			setOpen(true);
			changeCardHeightAndWidth(28, 20);
			return;
		}
		try {
			await Auth.forgotPasswordSubmit(email, code, password);
			await changeFormState(FormState.SignIn);
		} catch (error) {
	...
	useEffect(() => {
		changeCardHeightAndWidth(26, 20);
	}, []);
